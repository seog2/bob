...
<div id="container">
	<div id="content">
		<div class="orderGoods-wrap">
			<div class="orderGoods-title">
				<h1>1.주문확인</h1>
			</div>	
							...
							<tr cellpadding=40 align=center>
								...
								<td>
									<p id="p_final_totalPrice">
										<font>${final_total_order_price }원 </font>
				...
				<div class="detail_table">
					...
					<table class="list_view">
						<tbody>
							<tr >
								<td>
								   <input type="radio" id="pay_method" name="pay_method" value="creditCard" onClick="fn_pay_card()" checked> &nbsp;&nbsp; 신용카드 &nbsp;&nbsp;
								   <input type="radio" id="pay_method" name="pay_method" value="kakao"> &nbsp;&nbsp; 카카오페이(간편결제) &nbsp;&nbsp;
								   <input type="radio" id="pay_method" name="pay_method" value="naver"> &nbsp;&nbsp; 네이버페이(카드결제) &nbsp;&nbsp;
								   <input type="radio" id="pay_method" name="pay_method" value="naverPoint"> &nbsp;&nbsp; 네이버포인트(포인트결제) &nbsp;&nbsp;
								</td>
							</tr>
						<!-- 	<tr id="tr_pay_card">
								<td>
								...
								<br><Br>
								<strong> 유효기간 월 / 년 : <strong>  &nbsp;&nbsp;&nbsp;
								<input type = "text" id = "expireMonth" name = "expireMonth" maxLength = "2" size = "2">
								/
								<input type = "text" id = "expireYear" name = "expireYear" maxLength = "2" size = "2">
								
								<br><Br>
								<strong> 생년월일(6자리) : <strong>  &nbsp;&nbsp;&nbsp;
								<input type = "text" id = "birthday" name = "birthday" maxLength = "6" size = "6" >
								
								<br><Br>
								<strong> 비밀번호 2자리 : <strong>  &nbsp;&nbsp;&nbsp;
								<input type = "text" id = "cardPw" name = "cardPw" maxLength = "2" size = "2" >
								
								</td>
							</tr>
							<tr id="tr_pay_phone" style="visibility:hidden; display:none;" >
							  <td>
			...
			<ul class="orderGoods-btn" >
				<li><a href="javascript:fn_show_order_detail();"> 결제하기 </a></li>
				<li><a href="${contextPath}/main/main.do"> 쇼핑계속하기 </a></li>
			</ul>
			...
			<div id="popup_order_detail">
			  ...
			  <div class="detail_table">
				...
				<form name="order_info" method="post" accept-charset="euc-kr">
					...
					<!-- ※필수
					필수 항목 : 표준웹에서 값을 설정하는 부분으로 반드시 포함되어야 합니다 값을 설정하지 마십시오 -->
...
function m_Completepayment(FormOrJson, closeEvent) 
{
	...
	if (frm.res_cd.value == "0000") 
	{
		...
		*/
		...
		if ( payType == 'creditCard' ) {
			// 폼 name = order_info 서브밋 하면 된다.
			$("form[name=order_info]").attr("action", "/payment/kakao/keyin2.do" ) ;
		...
		} else if (payType == 'naver') { // 네이버로 오면 실행하는 코드	
			$("form[name=order_info]").attr("action", "/payment/naver/pay.do" ) ;
			$("form[name=order_info]").submit() ;
			return false;
				
		} else if (payType == 'naverPoint') { // 네이버포인트로 오면 실행하는 코드
			$("form[name=order_info]").attr("action", "/payment/naver/pay.do" ) ;
			$("form[name=order_info]").submit() ;
			return false;				
		}
